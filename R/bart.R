...
#' fitBARTSurvival
#'
#' @param trainXs A matrix/data.frame of X variables to be used in the model fitting. Columns should correspond to variables.
#' @param trainTarget A matrix/data.frame including columns named 'Event' and 'time_to_event' corresponding to a binary indicator of event/censoring and the time to event/censoring respectively.
#' @param testXs A matrix/data.frame of out-of-sample data to be used in model evaluation. The set of variables should be the same as those in trainXs.
#' @param pipelineRunInformation The object originally created by begin.pipeline.run.
#' @param sparse Set to TRUE if the sparsity inducing Dirichlet prior should be used for splitting rule variable selection.
...
#' @param nSkip The number of MCMC samples that should be treated as burn-in and discarded.
#' @param type 'pbart' (probit) or 'lbart' (logit)
#'
#' @return The fit BART survival model object of type survbart.
#' @export
fitBARTSurvival <- function(trainXs, trainTarget, testXs, pipelineRunInformation, sparse = FALSE, mcCores = 10, nTree = 100, k = 2, nSkip = 500, type = 'pbart') {
  tic('Fit BART survival model.')
  model <- mc.surv.bart(x.train = trainXs,
  ...
  if (pipelineRunInformation[['log']]) {
    ...
    saveRDS(model, paste0(pipelineRunInformation[['log folder path']], 'bart_survival_model_', pipelineRunInformation[['start timestamp']], '.rds'))
    toc(log = TRUE)
  }
...
cvBARTNYearOnset <- function(xs, target, nTreeList, nFolds, foldID, predictionTimePoint, pipelineRunInformation, sparse = FALSE, mcCores = 10, kList = c(2)) {
  cvTrainAndValidate <- function(nTree, k, validationFoldID) {
    ...
    bartSurvivalModel <- fitBARTSurvival(trainXs, trainTarget, validationXs, pipelineRunInformation, sparse, mcCores, nTree, k = k)
    ...
    survivalMeanPredictions <- sapply(1:nrow(validationXs), function(individual) {
    ...
    thresholdTTEResult <- thresholdTTE(validationTarget,
					  list(validationXs = validationXs,
    ...
    print(paste0('validationFoldID = ', validationFoldID, ', ntree = ', nTree, ', k = ', k, ', auc = ', aucResult))
  ...
  meanAUCs <- data.frame(nTree = integer(0), k = integer(0), auc = double(0), stringsAsFactors = FALSE)
