...
rfTrainAndTest <- function(trainXs,
                           trainY,
                           testXs,
                           testY,
                           metric,
...
rfCVIteration <- function(dataset,
                          labels,
                          foldIDs,
                          metric,
                          ntree,
  ...
  metricResults <- sapply(foldIDSet, function(foldID) {
    ...
    rfResult <- rfTrainAndTest(trainXs,
                               trainY,
                               testXs,
                               testY,
                               metric,
...
rfCVGridSearch <- function(xs,
  ...
  if ("mtrys" %in% extraParamNames) {
    ...
    mtrys <- if (!is.null(y) && !is.factor(y)) {
      max(floor(ncol(xs) / 3), 1)
      } else {
        floor(sqrt(ncol(xs)))
      }
  }
