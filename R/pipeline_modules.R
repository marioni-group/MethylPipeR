...
#' @param randomSeed A number to be used as the random seed in pipeline function calls.
#' @param log TRUE iff the pipeline log should be saved to file.
#' @param logFolderPath if log is TRUE, the log file will be saved in the folder specified by logFolderPath.
...
beginPipelineRun <- function(note, randomSeed = 42, log = TRUE, logFolderPath) {
  ...
  pipelineRunInformation[['start timestamp']] <- format(Sys.time(), '%Y_%m_%d_%H_%M_%S')
  if (log) {
    write(paste0(pipelineRunInformation[['start timestamp']], ': ', note), paste0(logFolderPath, 'run_notes.txt'), append = TRUE)
    tic(paste0('Finished pipeline run. Log files have been saved in ', logFolderPath))
  } else {
...
endPipelineRun <- function(pipelineRunInformation) {
  ...
  if (pipelineRunInformation[['log']]) {
    lapply(tic.log(), write, paste0(pipelineRunInformation[['log folder path']], 'pipeline_tictoc_', pipelineRunInformation[['start timestamp']], '.txt'), append = TRUE, ncolumns = 1000)
  }
}
...
#'
#' @param dataFilepath The filepath of the data to be loaded. Currently supports '.rds' and '.csv' endings
...
#' @param y A matrix/data.frame with a column named 'Event' corresponding to a binary label.
#' @param penalty 'lasso'/'elnet'/'ridge' A string specifying the penalty.
#' @param weights A vector of weights corresponding to the training data rows to be used in model fitting.
#' @param nFoldsCV The number of folds to be used in cross-validation (default is 3)
#' @param foldID A vector of numbers specifying a cross-validation fold. Each entry should correspond to a row in xs.
#' @param pipelineRunInformation The object originally created by begin.pipeline.run
#' @param modelLabel A string that will act as a label for this function call. The label will be used as a prefix in the 'penalty' and 'number of folds' entries in pipeline.run.information. It will also be used as a prefix for the saved object filenames in this function call.
#' @param prescaled A boolean specifying if the columns in xs have been scaled before the call to fitLogisticModel e.g. using scale(). If set to TRUE, this sets standardize = FALSE in the internal call to cv.glmnet
#' @param typeMeasure A string specifying the argument to type.measure in cv.glmnet. Defaults to 'default'.
#' @param penaltyFactor A vector of numbers indicating variable-specific weights for the lambda penalty.
...
fitLogisticModel <- function(xs, y, penalty, weights, nFoldsCV = 3, foldID = NULL, pipelineRunInformation, modelLabel, prescaled = FALSE, typeMeasure = 'default', penaltyFactor = NULL) {
  ...
  toc(log = TRUE)
  if (pipelineRunInformation[['log']]) {
    tic(paste0('Save logistic ', penalty, ' regression model. Label: ', modelLabel))
    saveRDS(model, paste0(pipelineRunInformation[['log folder path']], modelLabel, '_logistic_model_fit_', pipelineRunInformation[['start timestamp']], '.rds'))
    toc(log = TRUE)
  }
...
#' @param xs A matrix/data.frame corresponding to X variables. Columns should correspond to features.
#' @param pipelineRunInformation The object originally created by begin.pipeline.run.
#' @param modelLabel A string that will act as a label for this function call. The label will be used as a prefix for the saved object filenames in this function call.
#' @param predictType A string specifying the prediction type ('class' or 'response').
...
predictLogisticModel <- function(model, xs, pipelineRunInformation, modelLabel, predictType = 'class') {
  ...
  if (pipelineRunInformation[['log']]) {
    ...
    saveRDS(modelPredictions, paste0(pipelineRunInformation[['log folder path']], modelLabel, '_logistic_model_predictions_', pipelineRunInformation[['start timestamp']], '.rds'))
    toc(log = TRUE)
  }
...
calculateMetricsBinaryClassifier <- function(modelPredictions, y, pipelineRunInformation, modelLabel) {
  ...
  if (pipelineRunInformation[['log']]) {
    ...
    write.csv(modelMetrics, paste0(pipelineRunInformation[['log folder path']], modelLabel, '_logistic_model_metrics_', pipelineRunInformation[['start timestamp']], '.csv'))
    toc(log = TRUE)
  }
  return(modelMetrics)
}
...
#'
#' @param xs A matrix/data.frame corresponding to X variables. Columns should correspond to features.
#' @param target A matrix/data.frame with columns named 'Event' and 'time_to_event'. 'Event' should correspond to a binary label. 'time_to_event' should correspond to a positive numerical value representing the survival time (when Event == 1) or the time-to-censoring (when Event == 0)
#' @param penalty 'lasso'/'elnet'/'ridge' A string specifying the penalty.
#' @param weights A vector of weights corresponding to the training data rows to be used in model fitting.
#' @param nFoldsCV The number of folds to be used in cross-validation (default is 3)
#' @param foldID A vector of numbers specifying a cross-validation fold. Each entry should correspond to a row in xs.
...
fitCoxModel <- function(xs, target, penalty, weights, nFoldsCV = 3, foldID = FALSE, pipelineRunInformation, modelLabel, prescaled = FALSE, penaltyFactor = NULL) {
  ...
  if (pipelineRunInformation[['log']]) {
    ...
    saveRDS(model, paste0(pipelineRunInformation[['log folder path']], modelLabel, '_cox_model_fit_', pipelineRunInformation[['start timestamp']], '.rds'))
    toc(log = TRUE)
  }
  return(model)
}
...
predictCoxModel <- function(model, xs, pipelineRunInformation, modelLabel) {
  ...
  if (pipelineRunInformation[['log']]) {
    ...
    saveRDS(modelPredictions, paste0(pipelineRunInformation[['log folder path']], modelLabel, '_cox_model_predictions_', pipelineRunInformation[['start timestamp']], '.rds'))
    toc(log = TRUE)
  }
  return(modelPredictions)
}
...
getNonZeroCoefficients <- function(model, pipelineRunInformation, modelLabel) {
  ...
  if (pipelineRunInformation[['log']]) {
    ...
    write.csv(nonZeroModelCoefficients, paste0(pipelineRunInformation[['log folder path']], modelLabel, '_model_non_zero_model_coefficients_', pipelineRunInformation[['start timestamp']], '.csv'))
    toc(log = TRUE)
  }
...
#'
#' @param modelPredictions A vector of model predictions.
#' @param targetTable A matrix/data.frame with columns named 'Event' and 'time_to_event'. 'Event' should correspond to a binary label. 'time_to_event' should correspond to a positive numerical value representing the survival time (when Event == 1) or the time-to-censoring (when Event == 0)
#' @param covariatesTable A table that includes columns corresponding to covariates to be included in the model.
#' @param covariateColnames A named list with names corresponding to the covariate column names to be used in the model. The values in the list should correspond to the actual column name in covariatesTable
#' @param pipelineRunInformation The object originally created by beginPipelineRun.
#' @param modelLabel A string that will act as a label for this function call. The label will be used as a prefix for the saved object filenames in this function call.
#' @return A named list consisting of the following elements: 'coxPHModel' is the fit coxph model object (from the survival library). 'coxPHModelSummary' is the result from calling summary() on coxPHModel. 'pseudoR2' is the pseudo r-squared value for the model. 'hazardRatio' is the hazard ratio for the predictor variable.
fitCoxPH <- function(modelPredictions, targetTable, covariatesTable, covariateColnames, pipelineRunInformation, modelLabel) {
  tic(paste0('Fit coxph. Model label: ', modelLabel))
  coxPHData <- cbind.data.frame(time_to_event = targetTable[, 'time_to_event'], Event = targetTable[, 'Event'], modelPredictions = modelPredictions)
  ...
  coxPHModel <- coxph(as.formula(paste0('Surv(time_to_event, Event) ~ scale(modelPredictions)+',paste(names(covariateColnames), collapse = '+'))), coxPHData)

  coxPHModelSummary <- summary(coxPHModel)
  pseudoR2 <- coxPHModelSummary$rsq[[1]]
  hazardRatio <- coxPHModelSummary$conf.int[, -2]%>%cbind(pval = coxPHModelSummary$coefficients[, 5])%>%head(1)
  toc(log = TRUE)

  if (pipelineRunInformation[['log']]) {
    tic('Save coxph objects')
    saveRDS(coxPHModelSummary, paste0(pipelineRunInformation[['log folder path']], modelLabel, '_coxph_model_summary_', pipelineRunInformation[['start timestamp']], '.rds'))
    toc(log = TRUE)
  }
  return(list(coxPHModel = coxPHModel, coxPHModelSummary = coxPHModelSummary, pseudoR2 = pseudoR2, hazardRatio = hazardRatio))
}
...
#'
#' @param data A matrix/data.frame with a column named <class.colname> corresponding to a binary indicator of case/control.
#' @param classColname The name of the column corresponding to a binary indicator of case/control.
#' @param nFolds The number of folds to be assigned.
#' @param pipelineRunInformation The object originally created by beginPipelineRun.
#' @return A vector with elements corresponding to rows in data. Each element is a number between 1-nFolds and represents which cross-validation fold the row is assigned to.
#' @export
assignTrainingFolds <- function(data, classColname, nFolds, pipelineRunInformation) {
...
#'
#' @param targetToFilter A matrix or data.frame with a binary 1/0 column named 'dead' and a binary column named 'Event'
#' @param objectsToFilter A list of matrix or data.frame objects each with the same number of rows as targetToFilter. These objects will also have the same rows removed as targetToFilter.
#'
#' @return A list with elements targetFiltered (a matrix/data.frame) and objectsToFilter (a list of matrix/data.frames) corresponding to targetToFilter and objectsToFilter with rows corresponding to deaths in controls (dead == 1, Event == 0) removed
#' @export
removeDeathsFromControls <- function(targetToFilter, objectsToFilter) {
  ...
  objectsToFilter <- lapply(objectsToFilter, function(objectToFilter) {
    if (is.data.frame(objectToFilter) || is.matrix(objectToFilter)) {
      ...
      print('Error in remove.deaths.from.controls: object.to.filter not data.frame, matrix or vector')
    }
  })
  toc(log = TRUE)
  gc()
...
removeByThreshold <- function(targetToFilter, objectsToFilter, variableName, threshold, lessThan = TRUE) {
  comparisonString <- ifelse(lessThan, 'less than', 'greater than or equal to')
  tic(paste0('Remove rows with ', variableName, ' ', comparisonString, ' ', threshold))
  ...
  objectsToFilter <- lapply(objectsToFilter, function(objectToFilter) {
    if (is.data.frame(objectToFilter) || is.matrix(objectToFilter)) {
      ...
      print('Error in removeByThreshold: objectToFilter not data.frame, matrix or vector')
    }
  })
  toc(log = TRUE)
  gc()
...
thresholdTTE <- function(targetToFilter, objectsToFilter, threshold, eventColname = 'Event', tteColname = 'time_to_event') {
  ...
  objectsToFilter <- lapply(objectsToFilter, function(objectToFilter) {
    if (is.data.frame(objectToFilter) || is.matrix(objectToFilter)) {
      ...
      print('Error in remove.deaths.from.controls: objectToFilter not data.frame, matrix or vector')
    }
  })
  list(targetFiltered = targetToFilter,
       objectsFiltered = objectsToFilter,
       counts = list(casesKepty = sum(casesKeptIndex),
...
createTTEDensityPlot <- function(target, pipelineRunInformation, title = 'Time-to-event by Case/Control', xlabel = 'TTE', ylabel = 'Density') {
...
#'
#' @param X A matrix/data.frame with columns corresponding to the labels, all covariates and the score to be evaluated.
#' @param yColname The name of the column in X corresponding to the labels.
#' @param covColnames A list/vector of the column names corresponding to covariates.
#' @param scoreColname The name of the column in X corresponding to the score to be evaluated.
#' @param family The glm model family. Default is 'binomial' for logistic regression.
#' @param pipelineRunInformation The object originally created by beginPipelineRun.
#' @return A list containing results for the null and full models. For both model types, the formula string, glm model object, response, AUC and PRAUC are returned.
#' @export
incrementalTest <- function(X, yColname, covColnames, scoreColname, family = 'binomial', pipelineRunInformation) {
  ...
  if (pipelineRunInformation[['log']]) {
    saveRDS(nullModel, paste0(pipelineRunInformation[['log folder path']], 'incremental_logistic_null_model_', pipelineRunInformation[['start timestamp']], '.rds'))
    saveRDS(fullModel, paste0(pipelineRunInformation[['log folder path']], 'incremental_logistic_full_model_', pipelineRunInformation[['start timestamp']], '.rds'))
    saveRDS(nullModelResponse, paste0(pipelineRunInformation[['log folder path']], 'incremental_logistic_null_model_response_', pipelineRunInformation[['start timestamp']], '.rds'))
    saveRDS(fullModelResponse, paste0(pipelineRunInformation[['log folder path']], 'incremental_logistic_full_model_response_', pipelineRunInformation[['start timestamp']], '.rds'))
  }
